...
if __name__ == '__main__':
    ...
    L0 = -1/6 * np.ones((nx,ny,nz))
    L1 = -1/6 * np.ones((nx,ny,nz))
    ...
    grad_norm[np.where(grad_norm==0)] = 1e-12 ## to avoid dividing by zero
    ...
    den[np.where(den==0)] = 1e-12 ## to avoid dividing by zero
    ...
    depth[R[0],R[1],R[2]] = L1[R[0],R[1],R[2]]  + 1#+ L0[R[0],R[1],R[2]] -1
    ...
    #gaussian_filter(u, sigma=1, output=u)
    verts, faces, normals, values = measure.marching_cubes_lewiner(u, 2) ## 2-isosurface mesh

    m = trimesh.Trimesh(vertices=verts, faces=faces)
    m.export(args.output+'/dilated_surface_mesh.ply')


    texture = g3D.texture_spline_interpolation3D(verts, depth)

    g3D.display_mesh(verts, faces, normals, texture, os.path.join(args.output, "sulcal_depth.png"))
